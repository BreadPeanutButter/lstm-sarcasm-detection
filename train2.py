...
def train(model,
    ...
    for epoch in range(num_epochs):
        ...
        for (labels, (comment, comment_len)), _ in train_loader:
            ...
            # evaluation step
            if global_batch_num % eval_every == 0:
                model.eval()
                with torch.no_grad():                    
...
def plot_loss(metrics):
    ...
    plt.plot(epoch, train, 'bo', label='Train')
    plt.plot(epoch, valid, 'go', label='Valid')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
...
if __name__ == "__main__":
    ...
    train_loader=train_iter, valid_loader=valid_iter, num_epochs=1, eval_every=len(train_iter)//10) # Evaluate 10 times per epoch
